        ...
        py::dict scala_files_to_frequencies_16x128(std::string scl_filename, std::string kbm_filename)
        {
            auto s = Tunings::readSCLFile(scl_filename);
            Tunings::Tuning t;
            if (kbm_filename == "")
            ...
            {
                res[py::int_(i)] = t.frequencyForMidiNote(i);
            }
            return res;
        }
        ...
        PYBIND11_MODULE(_mtsespy, m)
        {
            ...
            m.def("scala_files_to_frequencies", &scala_files_to_frequencies,
                  ...
                  py::arg("kbm_filename") = "");
            m.def("scala_files_to_frequencies_16x128", &scala_files_to_frequencies_16x128,
                  "Build frequencies corresponding to given scala files", py::arg("scl_filename"),
                  py::arg("kbm_filename") = "");
